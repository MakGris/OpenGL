...
static ShaderProgramSource ParseShader(const std::string& filepath)
{
    ...
    while (getline(stream, line))
    {
        if (line.find("#shader") != std::string::npos)
        {
            ...
            {
            //если в файле с шейдерами в начале файла будут пустые строки, то массив будет пытаться заполниться по индексу -1, т.к. ShaderType будет = NONE, следовательно программа будет вылетать с ошибкой!
            ss[(int)type] << line << '\n';
            }
        }
...
int main()
{
    ...
    //Отображение текущей версии OpenGL
    std::cout << glGetString(GL_VERSION) << std::endl;

    //координаты позиций вершин (Vertex)
    float positions[] = {
        -0.5f, -0.5,    // 0
         0.5f, -0.5f,   // 1
         0.5f, 0.5f,    // 2
